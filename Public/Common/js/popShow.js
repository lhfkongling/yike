...
(function(e) {
	...
	var d = $,
		...
		m = c.find(".warnMsg"),
		n = c.find(".content"),
		o = c.find(".doBtn .ok"),//确定
		p = c.find(".doBtn .cancel"),//取消
		j = !1,
		f;
...
/**toast提示框
 * 使用方式：
 *      webToast("请登录！","bottom",1000);
 *      webToast("亲，你登录过了！","bottom");
 *      webToast("请登录",1000);
 *      webToast("哎，你登录了呀！");
 */
function webToast() {
...
//文本框提示
function inputTip(info,obj){
	console.log(info);
	obj.after('<font class="tip">'+info+'<i></i></font>');
	obj.next().show();
	setTimeout(removeTip,2000);

}
function removeTip(){
	$('.tableEdit .field li font.tip ').remove();
}